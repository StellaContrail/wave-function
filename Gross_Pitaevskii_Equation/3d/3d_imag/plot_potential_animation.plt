...
do for [i=0: 2*N] {
    ...
    if (i > N) {
        j = 2*N-i
        splot "data_potential_raw.txt" every :::50*j::(50*(j+1)-1) title "" with pm3d
    } else {
        splot "data_potential_raw.txt" every :::50*i::(50*(i+1)-1) title "" with pm3d
    }
}
unset output
set terminal wxt